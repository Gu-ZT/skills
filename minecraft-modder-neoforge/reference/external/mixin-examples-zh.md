# Mixin 示例（Fabric Wiki 中文）

> - 来源：https://wiki.fabricmc.net/zh_cn:tutorial:mixin_examples
...
> - 说明：原页代码块在抓取中丢失围栏与转义，本镜像已还原为标准 Java 代码块；「结果」片段中的 `+`/`-` 表示注入前后的差异
> - 镜像注记：MixinExtras 捕获局部变量的两个示例中，方法参数为 `arg2` / `arg` 而方法体调用 `arg1.doSomething4()`——原页笔误，照抄无法编译，应改为对应参数名

收录了较为常见的 Mixin 示例。此页面旨在帮助开发者快速学习 Mixin 的使用，建议在开始之前先查看 [Mixin 介绍](mixin-introduction-zh.md)。
...
注意：应该避免像这样两次强转 `this`。如果你是要使用来自目标类的方法或字段，使用 `@Shadow`。如果方法或字段是来自目标类的父类，可以直接将你的这个 mixin 继承那个父类。
